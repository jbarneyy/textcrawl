...
schema_list_items = types.FunctionDeclaration(
    name="list_items",
    description="List/display/show/provide character's inventory / stats / quests / equipment. Function will return a formatted string displaying the character's inventory / stats / quests / equipment.",
    parameters=types.Schema(
        type=types.Type.OBJECT,
        properties={},
        required=[]
...
schema_trade = types.FunctionDeclaration(
    name="trade",
    description="Open up trade between Player and Character. Only call if Player uses the 'trade' keyword, do not call if 'trade' is not in player response. Function will return True if trade is successful and False if trade is unsuccessful.",
    parameters=types.Schema(
        type=types.Type.OBJECT,
        properties={
            "Name": types.Schema(
                type=types.Type.STRING,
                description="String representing the name of the Character the Player is attempting to trade with. Only call function if Player explicitly requests trade with 'trade' keyword."
            )
        },
        required=["Name"]
    ),
...
schema_accept_quest = types.FunctionDeclaration(
    name="accept_quest",
    description="Player asks Character if they have any quests for them to attempt. Should only call if the Player specifically asks a Character's name if they have any quests available.",
    parameters=types.Schema(
        type=types.Type.OBJECT,
        properties={
            "Name": types.Schema(
                type=types.Type.STRING,
                description="Name of the Character that the Player is asking for a Quest from."
            )
        },
        required=["Name"]
    )
